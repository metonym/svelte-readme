...
describe("preprocessReadme", () => {
  test("ignores files that are not markdown", () => {
    const result = pre.markup({ content: "# Title", filename: "src/App.svelte" });
    expect(result).toBeNull();
  });

  test("ignores markdown files inside node_modules", () => {
    const result = pre.markup({ content: "# Title", filename: "node_modules/some-pkg/README.md" });
    expect(result).toBeNull();
  });
  ...
  test("replaces the REPO_URL marker with a link when repoUrl is set", async () => {
    ...
    const result = await withRepoUrl.markup({ content: "<!-- REPO_URL -->", filename: "README.md" });
    expect(result?.code).toContain('<a href="https://github.com/metonym/svelte-readme">GitHub repo</a>');
  });
  ...
  test("rewrites relative links against the default prefix URL (homepage/tree/master)", async () => {
    ...
    expect(code).toContain('<a href="https://github.com/metonym/svelte-readme/tree/master/foo.md">rel</a>');
  });

  test("rewrites relative links against a custom prefixUrl", async () => {
    const withPrefix = preprocessReadme({ name: NAME, svelte: SVELTE_ENTRY, prefixUrl: "https://example.com/base/" });
    const result = await withPrefix.markup({ content: "[rel](./foo.md)", filename: "README.md" });
    expect(result?.code).toContain('<a href="https://example.com/base/foo.md">rel</a>');
  });
  ...
  test("extracts a top-level <style> block out of the markdown body", async () => {
    const code = await markup("<style>\n  h1 { color: red; }\n</style>\n\n# Hi");
    expect(code).toMatch(/<style>\s*h1 \{ color: red; \}\s*<\/style>/);
...
`;
    ...
    expect(code).toContain('<ul><li><a href="#sub-section-a">Sub Section A</a></li>');
    expect(code).toContain('<li><a href="#sub-section-b">Sub Section B</a></li>');
  ...
  test("handles svelte code fences: default demo, `no-eval`, and `no-display`", async () => {
    ...
    expect(code).toContain('<div class="code-fence"><button>{count}</button></div>');
  ...
  test("falls back to raw output for a language Prism does not support", async () => {
    ...
    expect(code).toContain('<pre class="language-made-up-lang">{@html `plain text');
    expect(errorSpy).toHaveBeenCalledWith('Could not highlight language "made-up-lang".');
